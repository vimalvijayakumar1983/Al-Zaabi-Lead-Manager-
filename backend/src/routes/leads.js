...
function inferAiChatIntent(message) {
  const text = String(message || '').toLowerCase();
  if (/(next step|what next|next action|plan|priority)/.test(text)) return 'NEXT_ACTION';
  if (/(risk|blocker|stuck|objection|problem)/.test(text)) return 'RISK';
  if (/(score|probability|chance|conversion)/.test(text)) return 'SCORING';
  if (/(task|follow.?up|todo|reminder)/.test(text)) return 'TASKS';
  if (/(call|callback|disposition|phone)/.test(text)) return 'CALLS';
  if (/(email|whatsapp|message|outreach|nurture)/.test(text)) return 'OUTREACH';
  if (/(summary|overview|snapshot|brief)/.test(text)) return 'SUMMARY';
  return 'GENERAL';
}

function buildLeadAiChatReply(lead, message, history = []) {
  const now = Date.now();
  const safeTasks = Array.isArray(lead.tasks) ? lead.tasks : [];
  const safeNotes = Array.isArray(lead.notes) ? lead.notes : [];
  const safeCommunications = Array.isArray(lead.communications) ? lead.communications : [];
  const safeCallLogs = Array.isArray(lead.callLogs) ? lead.callLogs : [];
  ...
  const inboundCount = safeCommunications.filter((item) => item.direction === 'INBOUND').length;
  const outboundCount = safeCommunications.filter((item) => item.direction === 'OUTBOUND').length;
  const recentCallOutcomes = safeCallLogs
    .slice(0, 4)
    .map((log) => String(log.disposition || '').replace(/_/g, ' ').toLowerCase())
    .filter(Boolean);

  const latestSignals = [
    ...safeTasks.map((task) => task.updatedAt || task.createdAt),
    ...safeNotes.map((note) => note.createdAt),
    ...safeCommunications.map((item) => item.createdAt),
    ...safeCallLogs.map((log) => log.createdAt),
  ]
    .filter(Boolean)
    .map((value) => new Date(value).getTime())
    .filter((value) => Number.isFinite(value));
  const lastTouchAt = latestSignals.length > 0 ? Math.max(...latestSignals) : null;
  const staleDays = lastTouchAt ? Math.floor((now - lastTouchAt) / (24 * 60 * 60 * 1000)) : null;

  const intent = inferAiChatIntent(message);
  const leadName = getDisplayName(lead) || 'This lead';
  const stageName = lead?.stage?.name || formatLeadStatus(lead?.status);
  const score = Number(lead?.score || 0);
  const conversionProb = Number(lead?.conversionProb || 0);
  const summaryText = (lead?.aiSummary || '').trim();

  const historyContext = Array.isArray(history)
    ? history.filter((item) => item?.role === 'user').slice(-1)[0]?.content
    : '';

  let answer = `${leadName} is currently in ${stageName} with a lead score of ${score}/100 and conversion probability of ${Math.round(conversionProb * 100)}%. `;

  if (intent === 'SUMMARY') {
    answer += summaryText
      ? summaryText
      : `There are ${openTasks} open task(s), ${overdueTasks} overdue task(s), and communication split is ${inboundCount} inbound vs ${outboundCount} outbound.`;
  } else if (intent === 'NEXT_ACTION') {
    if (overdueTasks > 0) {
      answer += `Top priority is clearing ${overdueTasks} overdue task(s), then run a follow-up call to confirm decision timeline.`;
    } else if ((staleDays || 0) >= 2) {
      answer += `Activity is stale by ${staleDays} day(s); run a re-engagement call and send a short value-based follow-up message.`;
    } else {
      answer += 'Recommended next move: secure a concrete commitment milestone (meeting slot, callback window, or proposal review date).';
    }
  } else if (intent === 'RISK') {
    const riskChunks = [];
    if (overdueTasks > 0) riskChunks.push(`${overdueTasks} overdue task(s)`);
    if ((staleDays || 0) >= 3) riskChunks.push(`${staleDays} day(s) since meaningful activity`);
    if (recentCallOutcomes.some((outcome) => outcome.includes('not interested'))) {
      riskChunks.push('recent "not interested" call signal');
    }
    answer += riskChunks.length > 0
      ? `Main risk signals: ${riskChunks.join(', ')}.`
      : 'No major risk flags detected right now, but keep follow-up cadence consistent to avoid drop-off.';
  } else if (intent === 'SCORING') {
    answer += 'Score is influenced by activity freshness, task completion hygiene, stage progression, and communication engagement. ';
    answer += `Current engagement signal is ${inboundCount}:${outboundCount} (inbound:outbound), which should be improved with timely two-way conversations.`;
  } else if (intent === 'TASKS') {
    answer += `Create a focused sequence: 1) follow-up call task, 2) message/email task, 3) checkpoint task for decision timeline. `;
    if (overdueTasks > 0) answer += `Resolve ${overdueTasks} overdue task(s) first.`;
  } else if (intent === 'CALLS') {
    answer += recentCallOutcomes.length > 0
      ? `Recent call outcomes include ${recentCallOutcomes.join(', ')}. Use the next call to lock a specific next milestone.`
      : 'No recent call outcomes are logged yet. Start with a discovery or re-engagement call and capture disposition.';
  } else if (intent === 'OUTREACH') {
    answer += 'Use a concise message with one clear CTA. Example: share value update + ask for a specific 15-minute slot.';
  } else {
    answer += summaryText || 'I can help with next steps, risk detection, and drafting outreach for this lead.';
    if (historyContext) {
      answer += ` Based on your last question (“${String(historyContext).slice(0, 120)}”), focus on commitment clarity.`;
    }
  }

  const actions = [];
  if (overdueTasks > 0 || intent === 'TASKS') {
    actions.push({
      type: 'OPEN_TASK_MODAL',
      label: overdueTasks > 0 ? `Create task (${overdueTasks} overdue)` : 'Create follow-up task',
      priority: overdueTasks > 0 ? 'URGENT' : 'HIGH',
    });
  }
  if (!['WON', 'LOST'].includes(String(lead?.status || '')) && (intent === 'CALLS' || intent === 'NEXT_ACTION' || (staleDays || 0) >= 2)) {
    actions.push({
      type: 'OPEN_CALL_LOG_MODAL',
      label: 'Log follow-up call',
      priority: (staleDays || 0) >= 3 ? 'URGENT' : 'HIGH',
    });
  }
  if (intent === 'OUTREACH' || intent === 'GENERAL') {
    actions.push({
      type: 'OPEN_COMMUNICATIONS_TAB',
  ...
  const confidenceBase =
    50 +
    Math.min(14, safeTasks.length) +
    Math.min(12, safeCommunications.length) +
    Math.min(10, safeCallLogs.length) +
    Math.min(8, safeNotes.length);
  const confidence = clamp(confidenceBase - Math.min(14, Math.max(0, (staleDays || 0) - 1) * 2), 40, 97);
...
// ─── AI Copilot Chat for Lead ────────────────────────────────────
router.post('/:id/ai-chat', validate(aiChatRequestSchema), async (req, res, next) => {
  try {
    const where = { id: req.params.id, organizationId: { in: req.orgIds } };
    ...
    const lead = await prisma.lead.findFirst({
      ...
      include: {
        ...
        callLogs: {
          orderBy: { createdAt: 'desc' },
          take: 20,
          select: { id: true, disposition: true, notes: true, createdAt: true },
        },
      },
    });
    ...
    const reply = buildLeadAiChatReply(lead, req.validated.message, req.validated.history || []);
    return res.json({
      success: true,
      data: {
        ...reply,
